...
theorem le_of_all_pow_lt_succ {x y : ℝ} (hx : 1 < x) (hy : 1 < y)
  ...
  have hNp : 0 < N
  exact mod_cast (one_div_pos.mpr hxmy).trans hN
  have :=
    calc
...
theorem f_pos_of_pos {f : ℚ → ℝ} {q : ℚ} (hq : 0 < q)
    ...
    0 < f q := by
  have num_pos : 0 < q.num
  apply Rat.num_pos.mpr hq
  have hmul_pos :=
    calc
...
theorem fixed_point_of_pos_nat_pow {f : ℚ → ℝ} {n : ℕ} (hn : 0 < n)
  ...
  exact mod_cast H5 (a ^ n) (one_lt_pow ha1 hn.ne')
  have hh1 :=
    calc
...
theorem fixed_point_of_gt_1 {f : ℚ → ℝ} {x : ℚ} (hx : 1 < x)
  ...
  have h_big_enough : (1 : ℚ) < a ^ N - x
  apply lt_sub_iff_add_lt.mpr hN
  have h1 :=
    calc
