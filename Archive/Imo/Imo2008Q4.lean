...
theorem imo2008_q4 (f : ℝ → ℝ) (H₁ : ∀ x > 0, f x > 0) :
  ...
  intro H₂
  have h₀ : f 1 ≠ 0
  specialize H₁ 1 zero_lt_one; exact ne_of_gt H₁
  have h₁ : f 1 = 1
  ...
  intro x hx
  have h1xss : 1 * x = sqrt x * sqrt x
  rw [one_mul, mul_self_sqrt (le_of_lt hx)]
  ...
  have hfx_ne_0 : f x ≠ 0
  specialize H₁ x hx; exact ne_of_gt H₁
  field_simp at H₂ ⊢
  ...
  have hab : a * b > 0
  apply mul_pos ha hb
  have habss : a * b = sqrt (a * b) * sqrt (a * b)
  apply (mul_self_sqrt (le_of_lt hab)).symm
  ...
  · have hb_ne_0 : b ≠ 0
    apply ne_of_gt hb
    field_simp [hab₂] at H₂
    have H₃ : 2 * b ^ 4 * (a ^ 4 - 1) = 0
    linear_combination H₂
    have h2b4_ne_0 : 2 * b ^ 4 ≠ 0
    apply mul_ne_zero two_ne_zero (pow_ne_zero 4 hb_ne_0)
    have ha₁ : a ^ 4 = 1
    simpa [sub_eq_zero, h2b4_ne_0, hb_ne_0] using H₃
