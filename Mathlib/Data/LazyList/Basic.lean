...
instance : LawfulTraversable LazyList := by
  ...
  · induction' xs using LazyList.rec with _ _ _ _ ih
    · simp only [Functor.map, LazyList.traverse, Equiv.map, listEquivLazyList,
        Equiv.coe_fn_symm_mk, toList, Equiv.coe_fn_mk, ofList, pure]
...
theorem append_bind {α β} (xs : LazyList α) (ys : Thunk (LazyList α)) (f : α → LazyList β) :
  ...
  match xs with
  | LazyList.nil => simp only [append, Thunk.get, LazyList.bind]
  | LazyList.cons x xs =>
...
instance : LawfulMonad LazyList := LawfulMonad.mk'
  (bind_pure_comp := by
    ...
    induction' xs using LazyList.rec with _ _ _ _ ih
    · simp only [LazyList.bind, LazyList.traverse, Id.pure_eq]
    · simp only [bind.eq_2, append, traverse.eq_2, Id.map_eq, cons.injEq, true_and]; congr
    · congr; funext
      simpa only [Thunk.get, Thunk.pure, append] using ih ()
  )
  (pure_bind := by
    intros
    simp only [bind, pure, singleton, Thunk.pure, LazyList.bind, Thunk.get]
    apply append_nil)
  (bind_assoc := by
    intro _ _ _ xs _ _
    induction' xs using LazyList.rec with _ _ _ _ ih
