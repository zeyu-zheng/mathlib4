...
theorem iterate_mem_periodicOrbit (hx : x ∈ periodicPts f) (n : ℕ) :
    f^[n] x ∈ periodicOrbit f x := by simp [*]

@[simp]
theorem exists_iterate_of_mem_periodicPts (hx : x ∈ periodicPts f) : ∃ n, f^[n] x = x := by
  rw [← mem_periodicOrbit_iff hx]
  exact iterate_mem_periodicOrbit hx 0
