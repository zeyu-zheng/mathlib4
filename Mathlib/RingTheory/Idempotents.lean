...
/-- The corner associated to an element `e` that is left and right associative is
the multiplication-closed subset of all elements of the form `e * r * e`. -/
abbrev cornerOfAssoc [Mul R] (left_assoc : ∀ a b, e * a * b = e * (a * b))
    (right_assoc : ∀ a b, a * b * e = a * (b * e)) : Subsemigroup R where
  carrier := Set.range (e * · * e)
  mul_mem' := by rintro _ _ ⟨a, rfl⟩ ⟨b, rfl⟩; exact ⟨a * e * (e * b), by
    dsimp only; conv_rhs => rw [left_assoc, ← right_assoc, left_assoc]⟩
...
def corner : Subsemigroup R := cornerOfAssoc e (fun _ _ ↦ mul_assoc ..) fun _ _ ↦ mul_assoc ..
