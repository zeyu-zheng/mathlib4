...
noncomputable def flipₗᵢ' : (E →SL[σ₁₃] F →SL[σ₂₃] G) ≃ₗᵢ[𝕜₃] F →SL[σ₂₃] E →SL[σ₁₃] G where
  toFun := flip
  invFun := flip
  map_add' := flip_add
  map_smul' := flip_smul
...
variable (M₁ : Type) [SeminormedAddCommGroup M₁] [NormedSpace 𝕜 M₁] (M₂ : Type)
  [SeminormedAddCommGroup M₂] [NormedSpace 𝕜 M₂] (M₃ : Type) [SeminormedAddCommGroup M₃]
  [NormedSpace 𝕜 M₃] (M₄ : Type) [SeminormedAddCommGroup M₄] [NormedSpace 𝕜 M₄]
